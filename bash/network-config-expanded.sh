...
interlistall=$(lshw -class network |  awk '/logical name:/{print $3}')
#array to add all elements into that array list just need to convert the strings to array
declare -a interfacearray=($interlistall)
# converting the str to arr
len=${#interfacearray[*]}
i=0
#loop thrugh inteface list to process all things in the array
while [ $i -lt $len ]
do
    if [[ ${interfacearray[$i]} = lo* ]] ; 
	#statement to skip the report about loopback
	then continue ;
	fi
	#getting required things and adding it to var to print dynamically
	 ipv4_address=$(ip a s ${interfacearray[$i]} | awk -F '[/ ]+' '/inet /{print $3}')
	 ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')
	 network_address=$(ip route list dev ${interfacearray[$i]} scope link|cut -d ' ' -f 1)
	 network_number=$(cut -d / -f 1 <<<"$network_address")
	 network_name=$(getent networks $network_number|awk '{print $1}')
  #eco to print with custom name
  echo Interface ${interfacearray[$i]}:
  echo ===============
  echo Address         : $ipv4_address
  echo Name            : $ipv4_hostname
  echo Network Address : $network_address
  echo Network Name    : $network_name
  #incriment the I for next array processing
  i=$((i+1))
done
#ending the loop
#####
